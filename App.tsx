...
const VoiceRecorderModal = ({ isOpen, onClose, onSave, userProfile }: any) => {
    ...
    const handleSave = async () => {
        ...
        setLoading(true);
        try {
            const fileName = `report_${Date.now()}.webm`;
            ...
            const reportData = {
                ...
                content: `Rapport vocal du ${new Date().toLocaleDateString()}`,
                method: 'Voice',
                audioUrl: publicUrl,
                    ...
                    {audioBlob && !isRecording && (
                        <div className="flex flex-col gap-2">
                            ...
                            <div className="flex gap-2">
                                <button onClick={() => setAudioBlob(null)} className="flex-1 border py-3 rounded-xl text-gray-600 font-bold">Recommencer</button>
                                <button onClick={handleSave} disabled={loading} className="flex-1 bg-ebf-green text-white py-3 rounded-xl font-bold flex items-center justify-center gap-2">
    ...
    <div className="space-y-6 animate-fade-in">
        <div className="flex items-center justify-between">
            ...
            {!readOnly && (
                <div className="flex gap-2">
                    {onVoiceAdd && (
                        <button onClick={onVoiceAdd} className="bg-ebf-orange text-white px-4 py-2 rounded-lg font-bold shadow-md hover:opacity-90 flex items-center gap-2 transition transform hover:-translate-y-0.5">
                            <Mic size={18}/> Rapport Vocal
                        </button>
                    )}
                    {onAdd && (
                ...
                <tbody className="divide-y divide-gray-50">
                    {items.filter((i:any) => currentSite === Site.GLOBAL || i.site === currentSite).map((item: any) => (
                        <tr key={item.id} className="hover:bg-orange-50/20 transition-colors">
                            <td className="p-4">
                                {item.method === 'Voice' ? <div className="p-2 bg-red-50 text-red-500 rounded-full inline-block"><Mic size={14}/></div> : <div className="p-2 bg-blue-50 text-blue-500 rounded-full inline-block"><FileText size={14}/></div>}
                            </td>
                            ...
                            <td className="p-4 text-sm text-gray-600">
                                {item.method === 'Voice' ? (
                                    <audio src={item.audioUrl} controls className="h-8 max-w-[150px]" />
                                ) : (
...
const AppContent = ({ onLogout, userRole, userProfile, session }: any) => {
  ...
  useEffect(() => {
    ...
    const channel = supabase.channel('ebf-realtime-v3')
