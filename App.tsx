            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        ...
                        <p className="text-gray-500 text-sm">Prix, photos et fiches techniques</p>
                    </div>
                </div>
                ...
                {filteredItems.map((item: any) => (
                    <div key={item.id} className="bg-white dark:bg-gray-800 rounded-2xl shadow-sm border border-gray-100 overflow-hidden hover:shadow-xl transition-all duration-300 group flex flex-col">
                        <div className="relative h-48 bg-gray-100">
                            <img src={item.imageUrl || 'https://via.placeholder.com/400x300'} alt={item.name} className="w-full h-full object-cover group-hover:scale-105 transition-transform" />
                        ...
                        <div className="p-4 flex-1 flex flex-col">
                            <div className="flex justify-between items-start mb-2">
                                ...
                                <span className="text-sm font-black text-green-700">{item.price.toLocaleString()} F</span>
                            </div>
                            <p className="text-xs text-gray-500 mb-4 flex-1 line-clamp-2">{item.description}</p>
                            <div className="flex gap-2">
                                <button onClick={() => onAddToCart(item)} className="flex-1 bg-ebf-orange text-white py-2 rounded-lg font-bold text-xs flex items-center justify-center gap-2 hover:bg-orange-600 transition">
                    ...
                    <tbody className="divide-y divide-gray-50 dark:divide-gray-700">
                        {filteredItems.map((item: any) => (
                            <tr key={item.id} className="hover:bg-orange-50/20">
                                ...
                                <td className="p-4 text-sm text-gray-600 dark:text-gray-300">{item.description || item.specialty || item.role || '-'}</td>
...
// --- Report Mode Selector (Rapports) ---
const ReportModeSelector = ({ reports, onBack, onViewReport }: any) => {
    return (
        <div className="space-y-6 animate-fade-in">
            <div className="flex items-center gap-3">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <button className="bg-blue-600 text-white p-6 rounded-xl shadow-lg flex flex-col items-center gap-2 hover:scale-105 transition">
                    <Mic size={32} />
                    <span className="font-bold">Rapport Vocal</span>
                </button>
                <button className="bg-orange-600 text-white p-6 rounded-xl shadow-lg flex flex-col items-center gap-2 hover:scale-105 transition">
                    <FileText size={32} />
                    <span className="font-bold">Formulaire Détaillé</span>
                </button>
            </div>
        </div>
    );
...
const AppContent = ({ session, onLogout, userRole, userProfile }: any) => {
  ...
  const renderContent = () => {
     ...
     // TECHNICIENS
     if (currentPath === '/techniciens/interventions') return <ModulePlaceholder title="Planning Interventions" subtitle="Suivi terrain" items={interventions} onBack={() => handleNavigate('/techniciens')} color="bg-orange-500" currentSite={currentSite} currentPeriod={currentPeriod} onAdd={() => {}} onDelete={() => {}} />;
     if (currentPath === '/techniciens/rapports') return <ReportModeSelector reports={reports} onBack={() => handleNavigate('/techniciens')} onViewReport={() => {}} />;
     ...
     // COMPTABILITÉ
     if (currentPath === '/comptabilite/bilan') return <ModulePlaceholder title="Bilan Financier" subtitle="Journal de caisse" items={transactions} onBack={() => handleNavigate('/comptabilite')} color="bg-green-600" currentSite={currentSite} currentPeriod={currentPeriod} onAdd={() => {}} onDelete={() => {}} />;
     if (currentPath === '/comptabilite/rh') return <ModulePlaceholder title="Ressources Humaines" subtitle="Dossiers personnel" items={employees} onBack={() => handleNavigate('/comptabilite')} color="bg-purple-600" currentSite={currentSite} onAdd={() => {}} onDelete={() => {}} />;
     if (currentPath === '/comptabilite/paie') return <ModulePlaceholder title="Salaires" subtitle="Virements mensuels" items={payrolls} onBack={() => handleNavigate('/comptabilite')} color="bg-orange-500" onAdd={() => {}} onDelete={() => {}} />;

     // SECRÉTARIAT
     if (currentPath === '/secretariat/planning') return <div className="p-8 text-center text-gray-400">Planning Calendrier en cours...<button onClick={() => handleNavigate('/secretariat')} className="block mx-auto mt-4 text-ebf-orange font-bold">Retour</button></div>;
     if (currentPath === '/secretariat/clients') return <ModulePlaceholder title="Clients" subtitle="CRM" items={clients} onBack={() => handleNavigate('/secretariat')} color="bg-blue-500" currentSite={currentSite} onAdd={() => {}} onDelete={() => {}} />;
     if (currentPath === '/secretariat/caisse') return <ModulePlaceholder title="Caisse Menu" subtitle="Petites dépenses" items={caisse} onBack={() => handleNavigate('/secretariat')} color="bg-gray-600" onAdd={() => {}} onDelete={() => {}} />;
     ...
     if (currentPath === '/quincaillerie/fournisseurs') return <ModulePlaceholder title="Fournisseurs" subtitle="Partenaires" items={suppliers} onBack={() => handleNavigate('/quincaillerie')} color="bg-green-600" onAdd={() => {}} onDelete={() => {}} />;
     if (currentPath === '/quincaillerie/achats') return <ModulePlaceholder title="Bons d'Achat" subtitle="Historique commandes" items={purchases} onBack={() => handleNavigate('/quincaillerie')} color="bg-red-500" onAdd={() => {}} onDelete={() => {}} />;

     // ÉQUIPE
     if (currentPath === '/equipe') return <ModulePlaceholder title="Notre Équipe" subtitle="Membres du personnel" items={technicians} onBack={() => handleNavigate('/')} color="bg-indigo-500" currentSite={currentSite} onAdd={() => {}} onDelete={() => {}} />;

     return (
        <div className="p-10 text-center text-gray-400">
            <Wrench size={48} className="mx-auto mb-4 opacity-30" />
            <p className="font-medium">Module "{currentPath}" en construction.</p>
    ...
    <div className={`flex h-screen bg-gray-50 ${darkMode ? 'dark' : ''}`}>
        <aside className="fixed inset-y-0 left-0 z-40 w-64 bg-green-950 text-white transform lg:translate-x-0 transition-transform shadow-2xl flex flex-col">
            ...
            <nav className="p-4 space-y-2 flex-1 overflow-y-auto custom-scrollbar">
                {MAIN_MENU.map(item => (
                    <button key={item.id} onClick={() => handleNavigate(item.path)} className={`w-full flex items-center space-x-3 px-4 py-3 rounded-xl transition-all ${currentPath === item.path || (item.path !== '/' && currentPath.startsWith(item.path)) ? 'bg-ebf-orange text-white shadow-lg' : 'text-gray-400 hover:bg-white/5 hover:text-white'}`}>
                        <item.icon size={20} />
        ...
        <div className="flex-1 flex flex-col lg:ml-64 relative overflow-hidden">
             ...
             <main className="flex-1 overflow-y-auto p-4 md:p-8 bg-ebf-pattern relative">
                {renderContent()}
                
                {/* Panier Flottant Dynamique */}
                {cart.length > 0 && currentPath.includes('quincaillerie') && (
                    <div className="fixed bottom-6 left-1/2 -translate-x-1/2 bg-green-950 text-white px-6 py-4 rounded-2xl shadow-2xl flex items-center gap-6 z-50 animate-slide-in border-2 border-ebf-orange max-w-lg w-full lg:ml-32">
                        ...
                        <div className="flex-1">
                            ...
                            <p className="text-xs text-gray-400 truncate">Prêt pour commande/affectation</p>
                        </div>
                        <button onClick={() => alert('Panier validé !')} className="bg-ebf-orange text-white px-4 py-2 rounded-xl font-bold hover:bg-orange-600 transition shadow-lg">Valider</button>
                    </div>
                )}
             </main>
        </div>
